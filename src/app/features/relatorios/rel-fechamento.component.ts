...
import { dataBr, mesIso } from '../../shared/datas';
...
@Component({
  ...
  template: `
    <div class="filters">
      <div class="field"><label>Fechamento de</label><input class="input" type="month" [ngModel]="mes()" (ngModelChange)="mes.set($event); carregar()" /></div>
    ...
    @if (r(); as r) {
      <p class="muted small mb">Movimento de {{ dataBr(r.inicio) }} a {{ dataBr(r.fim) }}. Os bilhetes para entregar aos alunos ficam na aba "Bilhetes de cobrança".</p>
      <div class="grid grid-3 mb">
        ...
        <div class="card stat"><span class="label">Comprado fiado no período</span><span class="value">{{ r.totalFiadoNoMes | dinheiro }}</span></div>
      </div>
      <div class="table-wrap"><table class="table">
        <thead><tr><th>Aluno</th><th>Turma</th><th>Responsável</th><th class="num">Fiado no período</th><th class="num">Créditos</th><th class="num">Pagamentos</th><th class="num">Saldo atual</th><th></th></tr></thead>
        ...
        } @empty { <tr><td colspan="8" class="empty">Nenhum movimento no período.</td></tr> }</tbody>
      </table></div>
    }
  `,
})
export class RelFechamentoComponent {
  ...
  constructor() {
    this.cadastros.configuracao().subscribe(c => { this.mes.set(mesIso(c.cicloAtual.ano, c.cicloAtual.mes)); this.carregar(); });
  }
  carregar() {
