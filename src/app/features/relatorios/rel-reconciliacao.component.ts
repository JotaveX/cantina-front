  ...
  template: `
    ...
    @if (r(); as r) {
      <div class="grid grid-4 mb">
        ...
        <div class="card stat"><span class="label">Fichas</span><span class="value">{{ r.pedidosFeitos }}</span><span class="sub">{{ r.pedidosRetirados }} retiradas · {{ r.pedidosParciais }} parciais · {{ r.pedidosPendentes }} pendentes · {{ r.pedidosCancelados }} canceladas</span></div>
        <div class="card stat"><span class="label">Itens</span><span class="value">{{ r.itensVendidos }}</span><span class="sub">{{ r.itensRetirados }} entregues · {{ r.itensPendentes }} pendentes</span></div>
      </div>
      ...
      <div class="table-wrap"><table class="table">
        <thead><tr><th>Produto</th><th class="num">Vendidos</th><th class="num">Entregues</th><th class="num">Pendentes</th><th class="num">Baixas manuais</th><th class="num">Estoque no sistema</th><th class="num">Contagem física</th><th class="num">Diferença</th></tr></thead>
        <tbody>@for (p of r.produtos; track p.produtoId) {
          <tr><td>{{ p.nome }}</td><td class="num">{{ p.vendidos }}</td><td class="num">{{ p.retirados }}</td><td class="num" [class.neg]="p.pendentes > 0">{{ p.pendentes }}</td>
        ...
        } @empty { <tr><td colspan="8" class="empty">Nenhum movimento neste dia.</td></tr> }</tbody>
      </table></div>
    }
  `,
})
