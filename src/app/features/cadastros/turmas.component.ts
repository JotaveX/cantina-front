...
@Component({
  ...
  template: `
    <div class="page" style="max-width: 900px">
      <div class="page-head"><div><h1>Turmas</h1><p>Usadas para filtrar alunos e relatórios.</p></div></div>
      ...
      <div class="table-wrap">
        <table class="table">
          <thead><tr><th>Nome</th><th>Série</th><th>Situação</th><th></th></tr></thead>
          <tbody>
            @for (t of turmas(); track t.id) {
              <tr>
                <td>{{ t.nome }}</td><td>{{ t.serie || '—' }}</td>
            ...
            } @empty { <tr><td colspan="4" class="empty">Nenhuma turma cadastrada.</td></tr> }
          </tbody>
        </table>
      </div>
    </div>
...
export class TurmasComponent {
  ...
  form = this.fb.nonNullable.group({ nome: ['', Validators.required], serie: [''] });

  constructor() { this.carregar(); }
  carregar() { this.api.turmas().subscribe(t => this.turmas.set(t)); }

  editar(t: Turma) { this.editando.set(t); this.form.setValue({ nome: t.nome, serie: t.serie ?? '' }); }
